...
export default function MyFollowers() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0f172a] relative flex flex-col items-center py-8 md:py-16 px-2 overflow-x-hidden">
      ...
      <Helmet>
        <title>DevHub | My Followers</title>
      </Helmet>
      ...
      <div className="max-w-6xl w-full space-y-12">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-400 font-medium max-w-md mx-auto">
            These are the {followersList.length} people watching your journey.
          </p>
        </div>
              ...
              >
                <div className="flex items-center gap-4">
                  <img
                    ...
                    className="w-16 h-16 md:w-20 md:h-20 rounded-3xl object-cover"
                  />
                  <div>
                  ...
                  className={`flex items-center gap-2 px-4 py-3 rounded-2xl text-[12px] font-black transition-all 
                    ${
                      actionLoadingId === user.id
                        ? "bg-slate-200 dark:bg-slate-700 text-slate-500 cursor-not-allowed opacity-100" // كلاسات التحميل فقط
                        : followingIds.has(user.id)
                          ? "bg-slate-100 dark:bg-slate-800 text-slate-400 hover:bg-red-50 hover:text-red-500" // كلاسات الـ Unfollow
                          : "bg-primary text-white shadow-lg shadow-primary/30 hover:shadow-primary/50" // كلاسات الـ Follow
                    }`}
                >
                  {actionLoadingId === user.id ? (
                    <>
