      ...
      {collections.length > 0 ? (
        <div className="space-y-4">
          {collections.map((collection) => (
            <Link
              to={`/reading-list/${collection.id}`} // المسار اللي هنكرره في الـ Routes
              key={collection.id}
              className="block group bg-white rounded-2xl"
            >
              <article
                key={collection.id}
                  ...
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[10px] text-gray-400 mt-1 block">
                        Created {collection.created_at} | Updated {(collection.updated_at).substring(0, 10)}
                      </span>
                    </div>
                    <ChevronRight
                      size={24}
